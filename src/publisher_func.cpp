...
MinimalPublisher::MinimalPublisher() : Node("minimal_publisher"), count_(0) {
  //     std::string new_mesg_param =
  //     this->get_parameter("new_mesg").as_string();
  this->declare_parameter<std::string>(
      "new_mesg", "Hello, I am ROS Humble, the LTS Version :p ! ");
...
void MinimalPublisher::service_callback(
    const std::shared_ptr<ros2_beginner_tutorials::srv::ChangeString::Request>
        request,
    std::shared_ptr<ros2_beginner_tutorials::srv::ChangeString::Response>
        response) {
  RCLCPP_WARN(rclcpp::get_logger("rclcpp"), "Incoming request\nnew_string: %s",
              request->update.c_str());
...
void MinimalPublisher::make_transforms() {
  ...
  RCLCPP_INFO(this->get_logger(),
              "Broadcasting transformation from world to talker.");
